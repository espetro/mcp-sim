...
- OTel-native observability with a JSONL audit log: when `MCPSIM_OBSERVABILITY_ENABLED=true`, every MCP request becomes an OTel span (tool calls get `mcp_sim.<tool>` spans with `tool.name`, `audit.outcome`, `audit.latency_ms` and a SHA-256 hash of the arguments; raw arguments are never logged). Spans stream as JSON Lines (`mcp-sim.audit.v1` schema) to `~/.config/mcp-sim/audit.jsonl` by default, overridable via `MCPSIM_AUDIT_LOG` (`stdout`, `file`, or a path; 50 MB rotation, 5 generations). W3C `traceparent` headers on `/mcp` POSTs are accepted, so CI runners can parent mcp-sim spans into their own traces. Disabled by default with zero overhead. `MCPSIM_OTLP_ENDPOINT` is reserved for a future OTLP exporter. See `docs/observability.md`.
